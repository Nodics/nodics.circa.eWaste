    ...
    <div className="mobile-content" ref={content}>
      ...
      {help ? <section className="mobile-help">
        ...
        {step === "photo" && <>
          ...
          {!!preview && !ready && !busy && !journey.unsupportedItem && <div className="mobile-card mobile-recognition-recovery"><h2>{journey.impactRecovery ? "Update your impact estimate" : "Let’s check your photo"}</h2><p>{journey.impactRecovery ? "Your photo and details are saved. Retry the environmental estimate before review." : "Try identifying this photo again, or replace it with a clearer image. A new item is saved only after analysis succeeds."}</p><button className="mobile-secondary" onClick={() => void (journey.impactRecovery && draft ? journey.refreshImpact() : journey.retryAnalysis())}>{journey.impactRecovery ? "Retry impact estimate" : "Retry image analysis"}</button></div>}
