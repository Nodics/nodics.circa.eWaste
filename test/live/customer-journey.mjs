...
try {
  ...
  console.log(
    "PASS private upload and automatic actual Copilot photo analysis",
  );
  ...
  await page.getByLabel("Message Circa assistant").fill("It is a tablet");
  await page.getByLabel("Send message", { exact: true }).click();
  await page.getByText(/The item type is now/).waitFor({ timeout: 60000 });
  await page
    .getByRole("button", { name: "Edit details or replace photo" })
    .click();
  await expect(page.getByLabel("Item type", { exact: true })).toHaveValue(
    "TABLET_DEVICE",
  );
  ...
  await page
    .getByRole("button", { name: "Confirm and submit", exact: true })
    .waitFor();
  console.log("PASS correction automatically revalidates preview");
  ...
  if (process.env.CIRCA_REVIEW_SESSION_FILE) {
    ...
    await page
      .getByRole("spinbutton", { name: "Asking price in reward points" })
      .fill("12");
